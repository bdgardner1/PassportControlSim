...
# Selects the passenger action dependent on status
    ...
    def walk(self):
        if self.environment[self.y][self.x]< self.ptype_no: #If not in a valid queue loading zone
            ...
            if self.desk_queue[self.desk_choice] > self.desk_queue[self.desk_choice + 1] + 1:   # compare current desk queue with the following
                self.x = self.x + self.speed #Continue walking if next queue is at least 2 smaller
            else:
